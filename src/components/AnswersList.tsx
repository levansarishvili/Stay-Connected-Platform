export default function AnswerList() {
  return (
    <div className="flex flex-col gap-6">
      <ul className="flex flex-col gap-8">
        <li className="flex flex-col gap-4 p-6 rounded-lg bg-[#fff] cursor-pointer">
          <h2 className="text-2xl text-[#3C3C4399]">Answer Title</h2>
          <p className="text-xl">
            Answer Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Atque fugit quas amet possimus error, a ducimus, inventore doloribus
            aliquam expedita minus labore. Aut dolorem quidem odio hic dolore,
            autem ea!
          </p>
          <div className="flex gap-4 justify-between items-center">
            <div className="flex gap-4">
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                Frontend
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                SwiftUI
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                iOS
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-xl">Comment 1</span>
              <span className="text-[#3C3C4399] text-xl">Date Time</span>
            </div>
          </div>
        </li>

        <li className="flex flex-col gap-4 p-6 rounded-lg bg-white cursor-pointer">
          <h2 className="text-2xl text-[#3C3C4399]">Answer Title</h2>
          <p className="text-xl">
            Answer Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Atque fugit quas amet possimus error, a ducimus, inventore doloribus
            aliquam expedita minus labore. Aut dolorem quidem odio hic dolore,
            autem ea!
          </p>
          <div className="flex gap-4 justify-between items-center">
            <div className="flex gap-4">
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                Frontend
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                SwiftUI
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                iOS
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-xl">Comment 2</span>
              <span className="text-[#3C3C4399] text-xl">Date Time</span>
            </div>
          </div>
        </li>

        <li className="flex flex-col gap-4 p-6 rounded-lg bg-white cursor-pointer">
          <h2 className="text-2xl text-[#3C3C4399]">Answer Title</h2>
          <p className="text-xl">
            Answer Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Atque fugit quas amet possimus error, a ducimus, inventore doloribus
            aliquam expedita minus labore. Aut dolorem quidem odio hic dolore,
            autem ea!
          </p>
          <div className="flex gap-4 justify-between items-center">
            <div className="flex gap-4">
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                Frontend
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                SwiftUI
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                iOS
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-xl">Comment 3</span>
              <span className="text-[#3C3C4399] text-xl">Date Time</span>
            </div>
          </div>
        </li>

        <li className="flex flex-col gap-4 p-6 rounded-lg bg-white cursor-pointer">
          <h2 className="text-2xl text-[#3C3C4399]">Answer Title</h2>
          <p className="text-xl">
            Answer Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Atque fugit quas amet possimus error, a ducimus, inventore doloribus
            aliquam expedita minus labore. Aut dolorem quidem odio hic dolore,
            autem ea!
          </p>
          <div className="flex gap-4 justify-between items-center">
            <div className="flex gap-4">
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                Frontend
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                SwiftUI
              </span>
              <span className="flex text-xl items-center px-4 py-1 bg-[#EEF2FF] rounded-xl text-[#4F46E5]">
                iOS
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-xl">Comment 4</span>
              <span className="text-[#3C3C4399] text-xl">Date Time</span>
            </div>
          </div>
        </li>
      </ul>
    </div>
  );
}
